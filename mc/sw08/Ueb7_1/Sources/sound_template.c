 ...
 * \b Target:     MC Car \n
 *
 * $Id: sound_template.c 508 2013-10-31 08:58:42Z zajost $
 *--------------------------------------------------------------------
 */
#include "platform.h"
#include "sound.h"
...
static uint16 delay;                    // duration of the current tone or pause
...
interrupt void soundISRduration(void)
{
  ...
  // => clear interrupt flag & calc new compare value
...
void soundStart(void)
{
  ...
  TPM1C5V += noteCounterticks[melody[melodyPos].note];
  TPM1C5SC_MS5x = 1; // Output Compare
  TPM1C5SC_ELS5x = 1; // Toggle Output
  TPM1C5SC_CH5IE = 1;
